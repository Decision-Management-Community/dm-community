---
title: Pricing Policy
date: 2024-12-01
tags: [business-rules]
solutions:
  - title: ChatGPT/DMN
    author: Jacob Feldman
    affiliation: OpenRules
    url: https://dmcommunity.org/wp-content/uploads/2024/12/challenge2024dec.chatgpt-3.pdf
  - title: ChatGPT/Python
    author: Jack Jansonius
    url: https://dmcommunity.org/wp-content/uploads/2024/12/challenge2024dec.chatgpt.python.jj_.png
  - title: OpenRules
    author: Alex Mirtsyn
    affiliation: OpenRules
    url: https://dmcommunity.org/wp-content/uploads/2024/12/challenge2024dec.openrules-1.pdf
  - title: DT5GL/SQL
    author: Jack Jansonius
    url: https://dmcommunity.org/wp-content/uploads/2024/12/challenge2024dec.sql_.pdf
  - title: ChatGPT/SQL
    author: Jack Jansonius
    url: https://chatgpt.com/share/67828eb3-073c-8006-83ea-dfe91c552648
---

A company sells Standard and Premium products manufactured in different countries, using a mixed bracket volume-and-cost pricing policy for orders with multiple units:

| Units | Standard | Premium |
|---|---|---|
| 1–99 | $10/unit | $15/unit |
| 100–499 | $9/unit | $14/unit |
| 500+ | $8/unit | $13/unit |

For products originating outside the US or Canada, add a $50 handling fee per order. If the order includes hazardous material, increase the total price by 30%.

Example order: 200 Premium units from Germany with hazardous materials; 50 Standard units from the US without hazardous materials; 200 Premium units from Canada without hazardous materials; 150 Standard units from China with hazardous materials.

Send your solutions to [DecisionManagementCommunity@gmail.com](mailto:DecisionManagementCommunity@gmail.com), or [open a pull request](https://github.com/Decision-Management-Community/dm-community/blob/main/CONTRIBUTING.md) to add yours here.
